...
foreach $key (@objkeys) {
   ...
   if (scalar(@affects) && defined($a{"Wearable"})) {
      checkObjAffects(@affects);
   }
   checkObjWeightandValue($key);
...
sub checkObjWeightandValue($key) {
   my %ot = %{$obj{$key}};
   my $level = $ot{"Level"};
   my $weight = $ot{"Weight"};
   my $value = $ot{"Value"};
   ...
   if ($ot{"Object Type"} eq "Container") {
      $maxweight = int($level/15);
      $minweight = int($level/2);
      $maxvalue = $level * 10;
   }
   if ($ot{"Object Type"} eq "Drink Container") {
      $maxweight = int($level/15);
      $minweight = int($level/2);
      $maxvalue = $level * 10;
   }
